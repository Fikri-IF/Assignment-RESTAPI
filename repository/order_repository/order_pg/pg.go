...
func (orderPG *orderPG) DeleteOrder(orderId int) errs.Error {
	...
	if err != nil {
		tx.Rollback()
		return errs.NewInternalServerError("something went wrong when deleting items")
	}
	...
	if err != nil {
		tx.Rollback()
		return errs.NewInternalServerError("something went wrong when deleting order")
	}
